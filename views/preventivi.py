...
# --- 2. FUNZIONI DI RICERCA (OTTIMIZZATE SERVER-SIDE) ---

def search_clients(search_term: str):
    """Cerca i clienti direttamente su Supabase filtrando per agente"""
...
def search_articles(search_term: str):
    """Cerca gli articoli nel listino (40k+ righe) senza caricarli in memoria"""
    if not search_term or len(search_term) < 3:
        return []
    
    # Cerchiamo sia per codice che per descrizione usando l'operatore 'or' di Supabase
...
def show_preventivi():
    ...
    with st.expander("👤 Anagrafica", expanded=not bool(st.session_state.righe_preventivo)):
        cliente_sel = st_searchbox(
            search_clients, 
            placeholder="🔍 Cerca cliente in rubrica...", 
            key="search_cliente_prev"
        )
    ...
    with col_search:
        selected_article = st_searchbox(
            search_articles, 
            placeholder="Scrivi almeno 3 caratteri (codice o descrizione)...", 
            key=f"search_art_{st.session_state.search_key}", 
    ...
    if st.session_state.temp_item:
        ...
        with st.container():
            ...
            if item.get("tipo") == "NOTA_TESTO":
                ...
                testo_nota = st.text_area("Testo della nota (apparirà in grassetto nel preventivo)", value=item["DESCRIZIONE"])
                c1, c2 = st.columns(2)
                ...
                if metodo == "Sconti %":
                    ...
                    # Prendo sconti di default dal listino se presenti
                ...
                omaggio = st.checkbox("Articolo in Omaggio (Prezzo 0)", value=item.get('SCONTO_MERCE', False))
                if omaggio: pn = 0
    ...
    if st.session_state.righe_preventivo:
            ...
            if riga.get('tipo') == 'NOTA_TESTO':
                ...
                with st.container(border=True):
                    ...
                    if b1.button("✏️", key=f"edit_{idx}"):
                        ...
                        st.session_state.temp_item = edit_item; st.rerun()
                    if b2.button("🗑️", key=f"del_{idx}"):
                        st.session_state.righe_preventivo.pop(idx); st.rerun()

        st.divider()
        cn, cm = st.columns([2, 1])
        note_finali = cn.text_area("Note finali (condizioni di vendita, note per magazzino...)")
        cm.metric("TOTALE NETTO", f"€ {tot_n:,.2f}")
        
        num_prev = f"PREV-{datetime.now().strftime('%y%m%d-%H%M')}"
        
        # Preparazione testata per salvataggio
        ...
        if cli_obj:
            try:
                ...
                cp.download_button(
                    "📄 SCARICA PDF", 
                    data=pdf_b, 
                    file_name=f"{num_prev}_{cli_obj['ragione_sociale'][:15]}.pdf", 
        ...
        if cs.button("💾 SALVA E CHIUDI", type="primary", use_container_width=True):
            if not cli_obj: 
                st.error("⚠️ Errore: Seleziona un cliente prima di salvare!")
            else:
